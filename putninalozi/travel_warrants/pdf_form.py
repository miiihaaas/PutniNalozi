...
def create_pdf_form(warrant):
    ...
    #nastaviti ostale string-ove ukoliko ih bude bilo
    ...
    pdf.cell(0, 30, f'Sluzbeni Put #{warrant_id}', ln=True, align='C')

    pdf.set_font('times','', 12)
    pdf.cell(0, 5, f'Zaposleni: {name} {surname}', ln=True)
    pdf.cell(0, 5, f'Sa zadatkom: {with_task}', ln=True)
    if abroad_contry != "":
        pdf.cell(0, 5, f'Putovanje u inostranstvo: {abroad_contry}', ln=True)
    pdf.cell(0, 5, f'Relacija: {relation}', ln=True)
    pdf.cell(0, 5, f'Datum: {start_datetime} - {end_datetime}', ln=True)
