import os
from fpdf import FPDF
from num2words import num2words
from putninalozi.travel_warrants.functions import replace_serbian_characters, get_warrant_details


def create_pdf_form(warrant, br_casova, br_casova_ino, br_casova_start, br_casova_end, br_dnevnica, br_dnevnica_start, br_dnevnica_end, br_dnevnica_ino):
    warrant_number, name, surname, with_task, relation, abroad_contry, costs_pays, start_datetime, end_datetime, registracija_kolege_koji_vozi, automobil_kolege_koji_vozi, rod, startna_recenica = get_warrant_details(warrant)

    #za header
        # baza = folder gde se nalazi ovaj fajl
    base_dir = os.path.dirname(os.path.abspath(__file__))

    # idi jedan nivo gore (iz travel_warrants u putninalozi)
    project_root = os.path.dirname(base_dir)

    # napravi apsolutnu putanju do logo fajla
    company_logo = os.path.join(
        project_root,
        "static",
        "company_logos",
        warrant.travelwarrant_company.company_logo
    )
    company_name = warrant.travelwarrant_company.companyname
    company_address = warrant.travelwarrant_company.company_address + f" {warrant.travelwarrant_company.company_address_number}"
    company_zip_code = warrant.travelwarrant_company.company_zip_code
    company_city = warrant.travelwarrant_company.company_city
    company_state = warrant.travelwarrant_company.company_state
    company_pib = warrant.travelwarrant_company.company_pib
    company_mb = warrant.travelwarrant_company.company_mb
    company_phone = warrant.travelwarrant_company.company_phone
    company_mail = warrant.travelwarrant_company.company_mail
    company_site = warrant.travelwarrant_company.company_site

    currency = warrant.daily_wage_currency

    text_form = f'''{startna_recenica} upućuje se na službeni put dana {start_datetime}. {f'u' if '-' not in relation else 'na relaciji' } {relation} {f'({abroad_contry})'if abroad_contry !="" else ""} sa zadatkom: {with_task}.\n
Na službenom putu {'koristi' if warrant.together_with == '' else 'deli'} prevozno sredstvo {warrant.other if warrant.other != "" else f'{warrant.travelwarrant_vehicle.vehicle_brand if warrant.vehicle_id != None else f"{warrant.travelwarrant_personal.vehicle_brand if warrant.personal_vehicle_id !=None else automobil_kolege_koji_vozi}"} registracionih oznaka: {warrant.travelwarrant_vehicle.vehicle_registration if warrant.vehicle_id != None else ""}'}{warrant.travelwarrant_personal.vehicle_registration if warrant.personal_vehicle_id != None else ""}{registracija_kolege_koji_vozi}.\n
Dnevnica za ovo služebno putovanje pripada u iznosu od: {warrant.daily_wage:.2f} {currency}{f' / {warrant.daily_wage_abroad} {warrant.daily_wage_abroad_currency}' if warrant.abroad else ""}.\n
Na službenom putu će se zadržati najdalje do {end_datetime}, a u roku od 48h po povratku sa službenog puta i dolaska na posao, podneće pismeni izveštaj o obavljenom službenom poslu. Račun o učinjenim putnim troškovima podneti u roku od tri dana.\n
Putni troškovi padaju na teret: {costs_pays}.\n
{f'Odobravam isplatu akontacije u iznosu od: {warrant.advance_payment} {warrant.advance_payment_currency}.' if warrant.advance_payment > 0 else ""}\n
Nalogodavac: {warrant.principal_user.name} {warrant.principal_user.surname}.
'''


    class PDF(FPDF):
        def __init__(self, **kwargs):
            super(PDF, self).__init__(**kwargs)
            # baza = folder gde je pdf_form.py
            base_dir = os.path.dirname(os.path.abspath(__file__))

            # idi jedan nivo gore (iz travel_warrants u putninalozi)
            project_root = os.path.dirname(base_dir)

            # dođi do static/fonts
            font_path = os.path.join(project_root, "static", "fonts")

            # registruj fontove
            self.add_font('DejaVuSansCondensed', '',
                        os.path.join(font_path, 'DejaVuSansCondensed.ttf'),
                        uni=True)
            self.add_font('DejaVuSansCondensed', 'B',
                        os.path.join(font_path, 'DejaVuSansCondensed-Bold.ttf'),
                        uni=True)
        def header(self):
            # Logo
            self.image(company_logo, 1, 1, 25)
            # set font
            self.set_font('DejaVuSansCondensed', '', 8)
            # Kompanija
            self.cell(50, 3, f'                         {company_name}', ln=False, align='L')
            # PIB
            self.cell(0, 3, f'                         PIB: {company_pib}', ln=False, align='L')
            # web stranica
            self.cell(1, 3, f'                         web: {company_site}', ln=True, align='R')
            # adresa
            self.cell(50, 3, f'                         {company_address}', ln=False, align='L')
            # MB
            self.cell(0, 3, f'                         MB: {company_mb}', ln=False, align='L')
            # email
            self.cell(1, 3, f'                         email: {company_mail}', ln=True, align='R')
            # mesto
            self.cell(0, 3, f'                         {company_zip_code} {company_city}', ln=False, align='L')
            # telefon
            self.cell(1, 3, f'                         tel: {company_phone}', ln=True, align='R')
            # Država
            self.cell(8, 3, f'                         {company_state}', ln=True, align='L')
            # linija
            self.line(10, 30, 200, 30)
        def footer(self):
            pass

    pdf = PDF()
    pdf.alias_nb_pages()

    pdf.add_page()
    pdf.set_font('DejaVuSansCondensed','B', 16)
    if warrant.abroad:
        pdf.cell(0, 30, f'NALOG ZA SLUŽBENO PUTOVANJE U INOSTRANSTVU: {warrant_number}', ln=True, align='C')
    else:
        pdf.cell(0, 30, f'NALOG ZA SLUŽBENO PUTOVANJE U ZEMLJI: {warrant_number}', ln=True, align='C')

    pdf.set_font('DejaVuSansCondensed','', 12)
    pdf.multi_cell(0, 5, text_form, ln=True, border='B')

    pdf.cell(0, 5, f'Na osnovu prednjeg naloga {rod[3]} sam službeno putovanje i podnosim sledeći', ln=True, align='L')
    pdf.set_font('DejaVuSansCondensed','B', 16)
    pdf.cell(0, 20, f'PUTNI RAČUN', ln=True, align='C')

    # === TABELA 1: DNEVNICE ===
    pdf.set_fill_color(150, 150, 150)
    pdf.set_font('DejaVuSansCondensed','', 8)
    pdf.cell(80, 4, f'Dnevnice', border=1, ln=False, fill=True, align='L')
    pdf.cell(25, 4, f'Broj časova', border=1, ln=False, fill=True, align='C')
    pdf.cell(25, 4, f'Broj dnevnica', border=1, ln=False, fill=True, align='C')
    pdf.cell(30, 4, f'Po', border=1, ln=False, fill=True, align='C')
    pdf.cell(30, 4, f'Svega', border=1, ln=True, fill=True, align='C')
    pdf.multi_cell(80, 4, f'''Vreme odlaska: {warrant.start_datetime.strftime("%d/%m/%Y, %H:%M")}
Vreme povratka: {warrant.end_datetime.strftime("%d/%m/%Y, %H:%M")}''', border=1, new_y='TOP', align='L')
    pdf.cell(25, 8, f'{round(br_casova)}', border=1, new_y='LAST', align='C')
    pdf.cell(25, 8, f'{br_dnevnica}', border=1, new_y='LAST', align='C')
    pdf.cell(30, 8, f'{warrant.daily_wage:.2f} {currency}', border=1, new_y='LAST', align='R')
    pdf.cell(30, 8, f'{(float(warrant.daily_wage) * br_dnevnica):.2f} {currency}', border=1, new_x='LMARGIN', new_y='NEXT', align='R')
    if warrant.abroad:
        pdf.multi_cell(80, 4, f'''Izlazak iz države: {warrant.contry_leaving.strftime("%d/%m/%Y, %H:%M") if warrant.contry_leaving != None else '-'}
Povratak u državu: {warrant.contry_return.strftime("%d/%m/%Y, %H:%M") if warrant.contry_return != None else '-'}''', border=1, new_y='TOP', align='L')
        pdf.cell(25, 8, f'{round(br_casova_ino)}', border=1, new_y='LAST', align='C')
        pdf.cell(25, 8, f'{br_dnevnica_ino}', border=1, ln=False, align='C')
        pdf.cell(30, 8, f'{warrant.daily_wage_abroad:.2f} {warrant.daily_wage_abroad_currency}', border=1, ln=False, align='R')
        pdf.cell(30, 8, f'{(float(warrant.daily_wage_abroad) * br_dnevnica_ino):.2f} {warrant.daily_wage_abroad_currency}', border=1, ln=True, align='R')

    # === SEPARATOR ===
    pdf.cell(0, 6, '', ln=True)

    # === TABELA 2: TROŠKOVI (prazna za inicijalni nalog) ===
    pdf.set_fill_color(150, 150, 150)
    pdf.cell(60, 4, f'Tip troška', border=1, ln=False, fill=True, align='L')
    pdf.cell(95, 4, f'Opis troška', border=1, ln=False, fill=True, align='L')
    pdf.cell(35, 4, f'Iznos', border=1, ln=True, fill=True, align='C')
    for i in range(3):
        pdf.cell(60, 4, f'', border=1, ln=False, align='L')
        pdf.cell(95, 4, f'', border=1, ln=False, align='L')
        pdf.cell(35, 4, f'', border=1, ln=True, align='L')

    pdf.set_font('DejaVuSansCondensed', 'B', 8)
    pdf.cell(155, 4, f'Ukupni troškovi', border=1, ln=False, align='R')
    pdf.cell(35, 4, f'', border=1, ln=True, align='R')
    dnevnice_total = float(warrant.daily_wage) * br_dnevnica
    if warrant.abroad:
        dnevnice_total_ino = float(warrant.daily_wage_abroad) * br_dnevnica_ino
        pdf.cell(155, 4, f'Dnevnice', border=1, ln=False, align='R')
        pdf.cell(35, 4, f'{dnevnice_total:.2f} {currency} / {dnevnice_total_ino:.2f} {warrant.daily_wage_abroad_currency}', border=1, ln=True, align='R')
    else:
        pdf.cell(155, 4, f'Dnevnice', border=1, ln=False, align='R')
        pdf.cell(35, 4, f'{dnevnice_total:.2f} {currency}', border=1, ln=True, align='R')
    pdf.cell(155, 4, f'SVEGA', border=1, ln=False, align='R')
    pdf.cell(35, 4, f'', border=1, ln=True, align='R')
    pdf.cell(155, 4, f'Primljena akontacija', border=1, ln=False, align='R')
    pdf.cell(35, 4, f'{warrant.advance_payment:.2f} {warrant.advance_payment_currency}' if warrant.advance_payment > 0 else '', border=1, ln=True, align='R')
    pdf.cell(155, 4, f'OSTALO ZA ISPLATU/UPLATU', border=1, ln=False, align='R')
    pdf.cell(35, 4, f'', border=1, ln=True, align='R')

    pdf.set_font('DejaVuSansCondensed', '', 8)
    pdf.cell(0, 6, '', ln=True)
    pdf.multi_cell(190, 4, f'''U mestu {warrant.travelwarrant_company.company_city}, dana {warrant.start_datetime.strftime("%d/%m/%Y")}, {warrant.travelwarrant_user.name} {warrant.travelwarrant_user.surname}''', border=1, ln=True, align='C')


    # baza = folder gde se nalazi ovaj fajl
    base_dir = os.path.dirname(os.path.abspath(__file__))

    # idi jedan nivo gore (iz travel_warrants u putninalozi)
    project_root = os.path.dirname(base_dir)
    path = os.path.join(project_root, "static", "pdf_forms")
    file_name = replace_serbian_characters(f'{warrant_number} {company_name}-{name} {surname}.pdf')
    pdf.output(os.path.join(path, file_name))
    return file_name, text_form


def update_pdf_form(warrant, br_casova, br_casova_ino, br_casova_start, br_casova_end, br_dnevnica, br_dnevnica_start, br_dnevnica_end, br_dnevnica_ino, troskovi):
    warrant_number, name, surname, with_task, relation, abroad_contry, costs_pays, start_datetime, end_datetime, registracija_kolege_koji_vozi, automobil_kolege_koji_vozi, rod, startna_recenica = get_warrant_details(warrant)

    #za header
    # baza = folder gde se nalazi ovaj fajl
    base_dir = os.path.dirname(os.path.abspath(__file__))

    # idi jedan nivo gore (iz travel_warrants u putninalozi)
    project_root = os.path.dirname(base_dir)

    # napravi apsolutnu putanju do logo fajla
    company_logo = os.path.join(
        project_root,
        "static",
        "company_logos",
        warrant.travelwarrant_company.company_logo
    )
    company_name = warrant.travelwarrant_company.companyname
    company_address = warrant.travelwarrant_company.company_address + f" {warrant.travelwarrant_company.company_address_number}"
    company_zip_code = warrant.travelwarrant_company.company_zip_code
    company_city = warrant.travelwarrant_company.company_city
    company_state = warrant.travelwarrant_company.company_state
    company_pib = warrant.travelwarrant_company.company_pib
    company_mb = warrant.travelwarrant_company.company_mb
    company_phone = warrant.travelwarrant_company.company_phone
    company_mail = warrant.travelwarrant_company.company_mail
    company_site = warrant.travelwarrant_company.company_site

    currency = warrant.daily_wage_currency

    text_form = f'''{startna_recenica} upućuje se na službeni put dana {start_datetime}. {f'u' if '-' not in relation else 'na relaciji' } {relation} {f'({abroad_contry})'if abroad_contry !="" else ""} sa zadatkom: {with_task}.\n
Na službenom putu {'koristi' if warrant.together_with == '' else 'deli'} prevozno sredstvo {warrant.other if warrant.other != "" else f'{warrant.travelwarrant_vehicle.vehicle_brand if warrant.vehicle_id != None else f"{warrant.travelwarrant_personal.vehicle_brand if warrant.personal_vehicle_id !=None else automobil_kolege_koji_vozi}"} registracionih oznaka: {warrant.travelwarrant_vehicle.vehicle_registration if warrant.vehicle_id != None else ""}'}{warrant.travelwarrant_personal.vehicle_registration if warrant.personal_vehicle_id != None else ""}{registracija_kolege_koji_vozi}.\n
Dnevnica za ovo služebno putovanje pripada u iznosu od: {warrant.daily_wage:.2f} {currency}{f' / {warrant.daily_wage_abroad} {warrant.daily_wage_abroad_currency}' if warrant.abroad else ""}.\n
Na službenom putu će se zadržati najdalje do {end_datetime}, a u roku od 48h po povratku sa službenog puta i dolaska na posao, podneće pismeni izveštaj o obavljenom službenom poslu. Račun o učinjenim putnim troškovima podneti u roku od tri dana.\n
Putni troškovi padaju na teret: {costs_pays}.\n
{f'Odobravam isplatu akontacije u iznosu od: {warrant.advance_payment} {warrant.advance_payment_currency}.' if warrant.advance_payment > 0 else ""}\n
Nalogodavac: {warrant.principal_user.name} {warrant.principal_user.surname}.
'''


    class PDF(FPDF):
        def __init__(self, **kwargs):
            super(PDF, self).__init__(**kwargs)
            # baza = folder gde je pdf_form.py
            base_dir = os.path.dirname(os.path.abspath(__file__))

            # idi jedan nivo gore (iz travel_warrants u putninalozi)
            project_root = os.path.dirname(base_dir)

            # dođi do static/fonts
            font_path = os.path.join(project_root, "static", "fonts")

            # registruj fontove
            self.add_font('DejaVuSansCondensed', '',
                        os.path.join(font_path, 'DejaVuSansCondensed.ttf'),
                        uni=True)
            self.add_font('DejaVuSansCondensed', 'B',
                        os.path.join(font_path, 'DejaVuSansCondensed-Bold.ttf'),
                        uni=True)
        def header(self):
            # Logo
            self.image(company_logo, 1, 1, 25)
            # set font
            self.set_font('DejaVuSansCondensed', '', 8)
            # Kompanija
            self.cell(50, 3, f'                         {company_name}', ln=False, align='L')
            # PIB
            self.cell(0, 3, f'                         PIB: {company_pib}', ln=False, align='L')
            # web stranica
            self.cell(1, 3, f'                         web: {company_site}', ln=True, align='R')
            # adresa
            self.cell(50, 3, f'                         {company_address}', ln=False, align='L')
            # MB
            self.cell(0, 3, f'                         MB: {company_mb}', ln=False, align='L')
            # email
            self.cell(1, 3, f'                         email: {company_mail}', ln=True, align='R')
            # mesto
            self.cell(0, 3, f'                         {company_zip_code} {company_city}', ln=False, align='L')
            # telefon
            self.cell(1, 3, f'                         tel: {company_phone}', ln=True, align='R')
            # Država
            self.cell(8, 3, f'                         {company_state}', ln=True, align='L')
            # linija
            self.line(10, 30, 200, 30)
        def footer(self):
            pass

    pdf = PDF()
    pdf.alias_nb_pages()
    pdf.add_page()
    pdf.set_font('DejaVuSansCondensed', 'B', 16)
    if warrant.abroad:
        pdf.cell(0, 30, f'NALOG ZA SLUŽBENO PUTOVANJE U INOSTRANSTVU: {warrant_number}', ln=True, align='C')
    else:
        pdf.cell(0, 30, f'NALOG ZA SLUŽBENO PUTOVANJE U ZEMLJI: {warrant_number}', ln=True, align='C')

    pdf.set_font('DejaVuSansCondensed','', 12)
    pdf.multi_cell(0, 5, text_form.replace('\n', '\n'), ln=True, border='B')

    pdf.cell(0, 5, f'Na osnovu prednjeg naloga {rod[3]} sam službeno putovanje i podnosim sledeći', ln=True, align='L')
    pdf.set_font('DejaVuSansCondensed','B', 16)
    pdf.cell(0, 20, f'PUTNI RAČUN', ln=True, align='C')

    # === TABELA 1: DNEVNICE ===
    pdf.set_fill_color(150, 150, 150)
    pdf.set_font('DejaVuSansCondensed','', 8)
    pdf.cell(80, 4, f'Dnevnice', border=1, ln=False, fill=True, align='L')
    pdf.cell(25, 4, f'Broj časova', border=1, ln=False, fill=True, align='C')
    pdf.cell(25, 4, f'Broj dnevnica', border=1, ln=False, fill=True, align='C')
    pdf.cell(30, 4, f'Po', border=1, ln=False, fill=True, align='C')
    pdf.cell(30, 4, f'Svega', border=1, ln=True, fill=True, align='C')

    if warrant.abroad:
        # Red 1: Odlazak do izlaska iz države (domaća dnevnica)
        pdf.multi_cell(80, 4, f'''Vreme odlaska: {warrant.start_datetime.strftime("%d/%m/%Y, %H:%M")}
Izlazak iz države: {warrant.contry_leaving.strftime("%d/%m/%Y, %H:%M")}''', border=1, new_y='TOP', align='L')
        pdf.cell(25, 8, f'{round(br_casova_start)}', border=1, new_y='LAST', align='C')
        pdf.cell(25, 8, f'{br_dnevnica_start}', border=1, new_y='LAST', align='C')
        pdf.cell(30, 8, f'{warrant.daily_wage:.2f} {currency}', border=1, new_y='LAST', align='R')
        pdf.cell(30, 8, f'{(float(warrant.daily_wage) * br_dnevnica_start):.2f} {currency}', border=1, new_x='LMARGIN', new_y='NEXT', align='R')

        # Red 2: U inostranstvu (strana dnevnica)
        pdf.multi_cell(80, 4, f'''Izlazak iz države: {warrant.contry_leaving.strftime("%d/%m/%Y, %H:%M") if warrant.contry_leaving != None else '-'}
Povratak u državu: {warrant.contry_return.strftime("%d/%m/%Y, %H:%M") if warrant.contry_return != None else '-'}''', border=1, new_y='TOP', align='L')
        pdf.cell(25, 8, f'{round(br_casova_ino)}', border=1, new_y='LAST', align='C')
        pdf.cell(25, 8, f'{br_dnevnica_ino}', border=1, new_y='LAST', align='C')
        pdf.cell(30, 8, f'{warrant.daily_wage_abroad:.2f} {warrant.daily_wage_abroad_currency}', border=1, new_y='LAST', align='R')
        pdf.cell(30, 8, f'{(float(warrant.daily_wage_abroad) * br_dnevnica_ino):.2f} {warrant.daily_wage_abroad_currency}', border=1, new_x='LMARGIN', new_y='NEXT', align='R')

        # Red 3: Povratak u državu do kraja (domaća dnevnica)
        pdf.multi_cell(80, 4, f'''Povratak u državu: {warrant.contry_return.strftime("%d/%m/%Y, %H:%M")}
Vreme povratka: {warrant.end_datetime.strftime("%d/%m/%Y, %H:%M")}''', border=1, new_y='TOP', align='L')
        pdf.cell(25, 8, f'{round(br_casova_end)}', border=1, new_y='LAST', align='C')
        pdf.cell(25, 8, f'{br_dnevnica_end}', border=1, new_y='LAST', align='C')
        pdf.cell(30, 8, f'{warrant.daily_wage:.2f} {currency}', border=1, new_y='LAST', align='R')
        pdf.cell(30, 8, f'{(float(warrant.daily_wage) * br_dnevnica_end):.2f} {currency}', border=1, new_x='LMARGIN', new_y='NEXT', align='R')
    else:
        pdf.multi_cell(80, 4, f'''Vreme odlaska: {warrant.start_datetime.strftime("%d/%m/%Y, %H:%M")}
Vreme povratka: {warrant.end_datetime.strftime("%d/%m/%Y, %H:%M")}''', border=1, new_y='TOP', align='L')
        pdf.cell(25, 8, f'{round(br_casova)}', border=1, new_y='LAST', align='C')
        pdf.cell(25, 8, f'{br_dnevnica}', border=1, new_y='LAST', align='C')
        pdf.cell(30, 8, f'{warrant.daily_wage:.2f} {currency}', border=1, new_y='LAST', align='R')
        pdf.cell(30, 8, f'{(float(warrant.daily_wage) * br_dnevnica):.2f} {currency}', border=1, new_x='LMARGIN', new_y='NEXT', align='R')

    # === SEPARATOR između tabela ===
    pdf.cell(0, 6, '', ln=True)

    # === TABELA 2: TROŠKOVI ===
    pdf.set_fill_color(150, 150, 150)
    pdf.set_font('DejaVuSansCondensed','', 8)
    pdf.cell(60, 4, f'Tip troška', border=1, ln=False, fill=True, align='L')
    pdf.cell(95, 4, f'Opis troška', border=1, ln=False, fill=True, align='L')
    pdf.cell(35, 4, f'Iznos', border=1, ln=True, fill=True, align='C')

    ukupni_trosak = 0.0
    ukupni_trosak_ino = 0.0
    ino_currency = warrant.daily_wage_abroad_currency
    if troskovi != []:
        for trosak in troskovi:
            if trosak.amount_currency == currency:
                ukupni_trosak = ukupni_trosak + trosak.amount
            else:
                ino_currency = trosak.amount_currency
                ukupni_trosak_ino = ukupni_trosak_ino + trosak.amount
            pdf.cell(60, 4, f'{trosak.expenses_type}', border=1, ln=False, align='L')
            pdf.cell(95, 4, f'{trosak.description}', border=1, ln=False, align='L')
            pdf.cell(35, 4, f'{trosak.amount:.2f} {trosak.amount_currency}', border=1, ln=True, align='R')

    ukupni_trosak = round(ukupni_trosak, 2)
    ukupni_trosak_ino = round(ukupni_trosak_ino, 2)

    # Izračunavanje ukupnih dnevnica
    if warrant.abroad:
        dnevnice_total = round(float(warrant.daily_wage) * (br_dnevnica_start + br_dnevnica_end), 2)
        dnevnice_total_ino = round(float(warrant.daily_wage_abroad) * br_dnevnica_ino, 2)
    else:
        dnevnice_total = round(float(warrant.daily_wage) * br_dnevnica, 2)
        dnevnice_total_ino = 0.0

    # Sumarni redovi
    pdf.set_font('DejaVuSansCondensed', 'B', 8)
    pdf.cell(155, 4, f'Ukupni troškovi', border=1, ln=False, align='R')
    if warrant.abroad and ukupni_trosak_ino > 0:
        pdf.cell(35, 4, f'{ukupni_trosak:.2f} {currency} / {ukupni_trosak_ino:.2f} {ino_currency}', border=1, ln=True, align='R')
    else:
        pdf.cell(35, 4, f'{ukupni_trosak:.2f} {currency}', border=1, ln=True, align='R')

    pdf.cell(155, 4, f'Dnevnice', border=1, ln=False, align='R')
    if warrant.abroad:
        pdf.cell(35, 4, f'{dnevnice_total:.2f} {currency} / {dnevnice_total_ino:.2f} {warrant.daily_wage_abroad_currency}', border=1, ln=True, align='R')
    else:
        pdf.cell(35, 4, f'{dnevnice_total:.2f} {currency}', border=1, ln=True, align='R')

    svega = round(dnevnice_total + ukupni_trosak, 2)
    svega_ino = round(dnevnice_total_ino + ukupni_trosak_ino, 2)

    pdf.cell(155, 4, f'SVEGA', border=1, ln=False, align='R')
    if warrant.abroad:
        pdf.cell(35, 4, f'{svega:.2f} {currency} / {svega_ino:.2f} {warrant.daily_wage_abroad_currency}', border=1, ln=True, align='R')
    else:
        pdf.cell(35, 4, f'{svega:.2f} {currency}', border=1, ln=True, align='R')

    pdf.cell(155, 4, f'Primljena akontacija', border=1, ln=False, align='R')
    pdf.cell(35, 4, f'{warrant.advance_payment:.2f} {warrant.advance_payment_currency}' if warrant.advance_payment > 0 else '', border=1, ln=True, align='R')

    saldo = round(svega - (warrant.advance_payment if warrant.advance_payment_currency == currency else 0), 2)
    saldo_ino = round(svega_ino - (warrant.advance_payment if warrant.advance_payment_currency != currency else 0), 2)

    pdf.cell(155, 4, f'OSTALO ZA ISPLATU/UPLATU', border=1, ln=False, align='R')
    if warrant.abroad:
        pdf.cell(35, 4, f'{saldo:.2f} {currency} / {saldo_ino:.2f} {warrant.daily_wage_abroad_currency}', border=1, ln=True, align='R')
    else:
        pdf.cell(35, 4, f'{saldo:.2f} {currency}', border=1, ln=True, align='R')

    pdf.set_font('DejaVuSansCondensed', '', 8)
    pdf.cell(0, 6, '', ln=True)
    pdf.multi_cell(0, 8, f'''U mestu {warrant.travelwarrant_company.company_city}, dana {warrant.end_datetime.strftime("%d/%m/%Y")}, {warrant.travelwarrant_user.name} {warrant.travelwarrant_user.surname}''', border=1, ln=True, align='C')
    pdf.multi_cell(0, 4, f'', ln=True, align='L')

    saldo_slovima = num2words(abs(int(saldo)), lang="sr")

    iznos_tekst = f'{saldo:.2f} ({saldo_slovima}) {currency}'
    if warrant.abroad and saldo_ino != 0:
        saldo_ino_slovima = num2words(abs(int(saldo_ino)), lang="sr")
        iznos_tekst += f'; {saldo_ino:.2f} ({saldo_ino_slovima}) {warrant.daily_wage_abroad_currency}'

    potvrda = f'Potvrđujem da je službeno putovanje izvršeno prema ovom nalogu i odobravam obračun putnih troškova u iznosu od {iznos_tekst}.'

    if saldo > 0:
        potvrda += '\nNavedeni iznos se isplaćuje zaposlenom na teret poslodavca.'
    elif saldo == 0:
        potvrda += '\nPo ovom obračunu nema dodatne isplate niti obaveze povraćaja sredstava.'
    else:
        potvrda += '\nZaposleni je dužan da izvrši povraćaj navedenog iznosa poslodavcu.'

    potvrda += f'\nU mestu {warrant.travelwarrant_company.company_city}, dana {warrant.end_datetime.strftime("%d/%m/%Y")}.'

    pdf.multi_cell(0, 4, potvrda, ln=True, align='L')
    pdf.multi_cell(0, 4, f'''


Blagajnik: _____________________________________          Podnosilac računa: _____________________________________''', ln=True, align='C')

    # baza = folder gde se nalazi ovaj fajl
    base_dir = os.path.dirname(os.path.abspath(__file__))

    # idi jedan nivo gore (iz travel_warrants u putninalozi)
    project_root = os.path.dirname(base_dir)
    path = os.path.join(project_root, "static", "pdf_forms")
    file_name = replace_serbian_characters(f'{warrant_number} {company_name}-{name} {surname}.pdf')
    pdf.output(os.path.join(path, file_name))
    return file_name, text_form
